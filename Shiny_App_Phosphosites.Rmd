...
ui=fluidPage(
  ...
  br(),
  textInput("global_search", "Protein Search:", placeholder = "Search all groups (UP Acc. Nr. e.g., Q9Y2J0)"),
...
hits = df[
  ...
  "</p><p style='font-size: 0.9em; color: gray;'>Click to jump to group.</p>"
    ...
    HTML(paste0(
      "<p><b>Found in group(s):</b><br>",
      paste(matching_groups, collapse = "<br>"),
      "</p><p style='font-size: 0.9em; color: gray;'>Select group from dropdown to view details.</p>"
    ))
  })
  ...
  # Spalte 1 (Acc. Nr.) mit Link + Highlight
  ...
  # Andere Spalten (2–4): Nur Highlight
  if (search_term != "") {
    for (col in 2:4) {
  ...
  # Spalte 5 einfärben je nach P-Site
  ...
  tableHTML(df, widths = c(200, 200, 100, 300, 150), rownames = FALSE, escape = FALSE)
})
...
if (any(
  ...
  observeEvent(input$open_modal, {
    showModal(modalDialog(
      ...
      checkboxInput("show_realpos_modal", "Show Sequence Positions", value = TRUE),
  ...
  output$modal_viewer = renderReactable({
    ...
    group_nr = get_group_number(input$msa_choice)
    msa_df = msa_list[[input$msa_choice]]

    msa_df_msa = msa_df[, 6:(ncol(msa_df)-1)]
    colnames(msa_df_msa) = as.character(1:ncol(msa_df_msa))
    msa_df_msa$Protein = if (isTRUE(input$show_realpos_modal)) {
  rep(";)", nrow(msa_df_msa))
} else {
  msa_df[, 1]
}
